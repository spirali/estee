...
def montage(width=50):
    ...
    img = tg.new_task("mImgTbl", duration=8)
...
def cybershake(width=50):
    ...
    peakval = gen_level(tg, width, "PeakValCalcOkaya-{}", lambda: normal(1, 0.5), lambda: 0.01)
...
def epigenomics(width=50):
    ...
    filter = gen_level(tg, width, "filter_Contams-{}",
                       lambda: normal(1.5, 1),
                       lambda: [normal(5, 2) for _ in range(2)])
    ...
    merge = tg.new_task(name="mapMerge", duration=normal(10, 2), outputs=[normal(20, 2), 0.5, 0.1])
    merge.add_inputs(map_chr)

    maqindex = tg.new_task(name="maqindex", duration=0.02, output_size=0.4)
    maqindex.add_input(merge.outputs[0])
    tg.new_task(name="pileup", duration=normal(6000, 200), output_size=3).add_input(maqindex)
...
def sipht(copies=2):
    ...
    def create_subgraph():
        parsers = gen_level(tg, 20, "Parser-{}", lambda: normal(1, 0.2), lambda: normal(0.1, 0.02))
        parser_concat = tg.new_task(name="Parser-concat", duration=0.05,
                                    output_size=normal(1.5, 0.1))
        parser_concat.add_inputs(parsers)

        find_term = tg.new_task(name="Findterm", duration=normal(1800, 100),
                                outputs=[0, 0, normal(20, 1), normal(0.2, 0.1), 0,
                                         normal(4, 1), normal(4, 1), normal(6, 1)])
        rna_motif = tg.new_task(name="RNAMotif", duration=normal(38, 2), outputs=[0, 1])
        transterm = tg.new_task(name="Transterm", duration=normal(50, 2), outputs=[0, 0.5])
        blast = tg.new_task(name="Blast", duration=normal(1900, 100), outputs=[0, 0, 4.5])

        top = [find_term, rna_motif, transterm, blast]

        srna = tg.new_task(name="SRNA", duration=normal(450, 50), outputs=[normal(0.5, 0.2)
                                                                           for _ in range(10)])
        for t in top:
            srna.add_inputs(t.outputs)
        output = srna.outputs[-1]

        blast = tg.new_task(name="BlastQRNA", duration=normal(1200, 100),
                            outputs=[normal(0.5, 1) for _ in range(8)])
        blast2 = tg.new_task(name="Blast_paralogues", duration=normal(5, 1), output_size=0.9)
        ffn_parse = tg.new_task(name="FFN_Parse", duration=normal(1, 1), output_size=0.9)
        blast_synteny = tg.new_task(name="Blast_synteny", duration=normal(30, 5), output_size=1)
        blast_synteny.add_input(ffn_parse)
        blast_candidate = tg.new_task(name="Blast_candidate", duration=normal(6, 1),
                                      outputs=[0] * 3)
        level = [blast, blast2, ffn_parse, blast_synteny, blast_candidate]
        for t in level:
            t.add_input(output)

        annotate = tg.new_task(name="SRNA_annotate", duration=normal(1, 2),
